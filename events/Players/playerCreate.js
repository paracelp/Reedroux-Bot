...
module.exports = {
    ...
    run: async (client, player) => {
        ...
        if (!message) return;
       const but1 = new MessageButton().setCustomId(`${message.guildId}pause`).setEmoji(client.emoji.pause).setStyle('SECONDARY').setDisabled(false)
                ...
                const but4 = new MessageButton().setCustomId(`${message.guildId}voldown`).setEmoji(client.emoji.volumelow).setStyle('SECONDARY').setDisabled(false)
                const but5 = new MessageButton().setCustomId(`${message.guildId}volup`).setEmoji(client.emoji.volumehigh).setStyle('SECONDARY').setDisabled(false)
