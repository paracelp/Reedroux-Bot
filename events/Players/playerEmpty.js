...
module.exports = {
  ...
  run: async (client, player) => {
    ...
    if (TwoFourSeven) {
      client.channels.cache.get(player.textId)?.send({
        embeds: [
          new MessageEmbed()
            ...
            .setDescription('**Queue ended! 24/7 is \`ON\`, I am not Leaving the voice channel.**')
            .setTimestamp(),
        ],
      ...
      client.channels.cache.get(player.textId)?.send({
        embeds: [
          new MessageEmbed()
            .setColor(client.embedColor)
            .setDescription('**Queue ended! 24/7 is \`OFF\`, I am Leaving the voice channel.**'),
        ],
      }).then(msg => { setTimeout(() => { msg.delete() }, 10000) });
    ...
    if (!message) return;
    const attachment = new MessageAttachment('bg-n.png');
    await message.edit({
      embeds: [
        new MessageEmbed().setColor(client.embedColor).setTitle(`Nothing playing right now in this server!`).setDescription(`[Invite](${client.config.links.invite}) - [Support Server](${client.config.links.support})`).setImage(`attachment://bg-n.png`)],
     files: [attachment], 
                       }).catch(() => { });
