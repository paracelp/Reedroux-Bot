  ...
  run: async (client, message) => {
    ...
    if (message.content.match(mention)) {
      const row = new MessageActionRow().addComponents(
        ...
        new MessageButton().setLabel('Support Server').setStyle('LINK').setURL(client.config.links.support),
        new MessageButton().setLabel('Premium').setStyle('LINK').setURL(client.config.links.premium)
      );
      const embed = new MessageEmbed()
        .setColor(client.embedColor)
        .setAuthor({ name: `${client.user.username}`, iconURL: client.user.displayAvatarURL(), url: client.config.links.invite.premium })
        .setDescription(`**Prefix : \`/ Slash Commands\`\n Voice region: \`us-central\`\n Server Id: \`${message.guild.id}\`**\n\n You can play music by joining a voice channel and typing \`/play\`.\nType \`/help\`, \`${prefix}help\` to get all commands help menu.`)
      .setFooter(`Developed by reedrouxmusic#0`);
